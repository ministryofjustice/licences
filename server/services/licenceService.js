...
module.exports = function createLicenceService(
    licenceClient,
    ...
    async function getLicence(nomisId, {populateConditions = false} = {}) {
        try {
            ...
            if (
                populateConditions &&
                !isEmpty(formattedLicence.additionalConditions)
            ) {
                return await populateLicenceWithCondtions(
                    formattedLicence,
                    status
                );
            }
    ...
    async function createLicence(nomisId, data = {}) {
        ...
        try {
            return await licenceClient.createLicence(
                nomisId,
                licence,
                'STARTED'
            );
    ...
    async function updateAddress(data = {}) {
        ...
        try {
            return await licenceClient.updateSection(
                'dischargeAddress',
                nomisId,
                address
            );
    ...
    async function updateLicenceConditions(data = {}) {
        try {
            const nomisId = data.nomisId;
            const selectedConditions = await licenceClient.getAdditionalConditions(
    ...
    async function getEstablishment(nomisId) {
        try {
            ...
            return await establishmentsClient.findById(
                record.licence.agencyLocationId
            );
        } catch (error) {
            console.error(error, 'Error during send licence');
            throw error;
    ...
    async function populateLicenceWithCondtions(licence, status) {
        ...
        const conditionsSelected = await licenceClient.getAdditionalConditions(
            conditionIdsSelected
        );

        return {
            licence: addAdditionalConditionsAsObject(
                licence,
                conditionsSelected
            ),
            status
