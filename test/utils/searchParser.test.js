...
describe('parseSearchTerms', () => {
  describe('valid searches', () => {
    ...
    inputsAndOutputs.forEach(([label, searchTerms, expected]) => {
      test(`should parse ${label}`, () => {
        expect(parseSearchTerms(searchTerms).query).toEqual(expected)
      })
    })
  })
  ...
  describe('invalid searches', () => {
    ...
    inputsAndOutputs.forEach(([label, searchTerms, expected]) => {
      test(`should report error when ${label}`, () => {
        expect(parseSearchTerms(searchTerms).error).toEqual(expected)
      })
    })
  })
})
