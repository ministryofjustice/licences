        ...
        describe('eligibility', () => {
            ...
            describe('excluded', () => {
                ...
                options.forEach(option => {
                    it(`should return ${JSON.stringify(option.outcome)} for ${JSON.stringify(
                        option.formResponse
                    )}`, () => {
    ...
    describe('validateFormGroup', () => {
            ...
            context('bass referral needed', () => {
                ...
                bassOptions.forEach(option => {
                    it(`should return ${JSON.stringify(option.outcome)} for ${JSON.stringify(option.licence)}`, () => {
                        const { outcome, licence } = option
                        expect(
                            service.validateFormGroup({ licence, stage, decisions: { bassReferralNeeded: true } })
                        ).to.eql(outcome)
                    })
                })
            })
        ...
        describe('processing_ro', () => {
            ...
            context('address not rejected', () => {
                options.forEach(option => {
                    it(`should return ${JSON.stringify(option.standardOutcome)} for ${JSON.stringify(
                        option.licence
                    )}`, () => {
                        const { standardOutcome, licence, decisions } = option
                        expect(service.validateFormGroup({ licence, stage, decisions: decisions || {} })).to.eql(
                            standardOutcome
                        )
                    })
                })
            ...
            context('address review failed', () => {
                options.forEach(option => {
                    it(`should return ${JSON.stringify(option.outcome)} for ${JSON.stringify(option.licence)}`, () => {
                        ...
                        expect(service.validateFormGroup({ licence, stage, decisions: decs })).to.eql(
                            addressReviewFailedOutcome
                        )
                    })
                })
            ...
            context('bass area suitable', () => {
                ...
                bassOptions.forEach(option => {
                    it(`should return ${JSON.stringify(option.bassOutcome)} for ${JSON.stringify(
                        option.licence
                    )}`, () => {
                        const { bassOutcome, licence } = option
                        expect(
                            service.validateFormGroup({ licence, stage, decisions: { bassAreaNotSuitable: true } })
                        ).to.eql(bassOutcome)
                    })
                })
            })
