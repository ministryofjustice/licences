...
describe('/licencesWithCOM/', () => {
  ...
  describe('GET', () => {
    ...
    test('should throw if submitted by non-authorised user', () => {
      ...
      return request(app).get('/admin/licencesWithCOM').expect(403)
    })
  })
  ...
  describe('POST', () => {
    ...
    test('should throw if submitted by non-authorised user', () => {
      const app = createApp('roUser')
      return request(app).post('/admin/licencesWithCOM').expect(403)
    })
  })

  const createApp = (user) => startRoute(createAdminRoute(licenceSearchService), '/admin/licencesWithCOM', user)
})
