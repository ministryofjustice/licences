...
describe('service healthcheck', () => {
  ...
  describe('check unhealthy', () => {
    test('should throw error from api', async () => {
      fakeServiceApi
        .get('/ping')
        .thrice()
        .reply(500)
  ...
  describe('check healthy retry test', () => {
    test('Should retry twice if request fails', async () => {
      fakeServiceApi
        .get('/ping')
    ...
    test('Should retry twice if request times out', async () => {
      fakeServiceApi
        .get('/ping')
    ...
    test('Should fail if request times out three times', async () => {
      fakeServiceApi
        .get('/ping')
        .delay(10000) // delay set to 10s, timeout to 900/3=300ms
        .reply(200, { failure: 'one' })
