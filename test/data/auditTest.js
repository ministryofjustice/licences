...
describe('Audit', () => {
  ...
  describe('getEvents', () => {
    ...
    it('should pass in the correct query', () => {
      ...
      return result.then(() => {
        expect(queryStub.getCall(0).args[0].text).to.eql('select * from audit where action = $1 and details @> $2')
    ...
    context('Start and end dates are passed in', () => {
      ...
      it('should handle just a start date', () => {
        const result = audit().getEvents('ACTION', { filter1: 'a' }, moment('13-03-1985', 'DD-MM-YYYY'), null)
      ...
      it('should handle just an end date', () => {
        const result = audit().getEvents('ACTION', { filter1: 'a' }, null, moment('15-03-1985', 'DD-MM-YYYY'))
