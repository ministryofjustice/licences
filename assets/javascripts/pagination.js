...
function paginate() {
    ...
    function updateState() {
        ...
        state.lastIndex = Math.min(state.firstIndex + state.numberToShow, state.totalNumber) - 1
    }
    ...
    function addPaginationInfo() {
        $('#paginationInfo').html(
            'Offenders ' +
                String(state.firstIndex + 1) +
                ' - ' +
                String(state.lastIndex + 1) +
                ' of ' +
                String(state.totalNumber)
        )
