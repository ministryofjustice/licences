...
var list=[]; var cells=rows[0].cells; var l=cells.length; for (var i=0; i<l; i++) {
...
var l=parsers.length; var node=false; var nodeValue=false; var keepLooking=true; while (nodeValue==''&&keepLooking) {
...
} var totalRows=(table.tBodies[0]&&table.tBodies[0].rows.length)||0; var totalCells=(table.tBodies[0].rows[0]&&table.tBodies[0].rows[0].cells.length)||0; var parsers=table.config.parsers; var cache={row: [], normalized: []}; for (var i=0; i<totalRows; ++i) {
var c=$(table.tBodies[0].rows[i]); var cols=[]; if (c.hasClass(table.config.cssChildRow)) {
...
} var c=cache; var r=c.row; var n=c.normalized; var totalRows=n.length; var checkCell=(n[0].length-1); var tableBody=$(table.tBodies[0]); var rows=[]; for (var i=0; i<totalRows; i++) {
...
var arr=[]; var r=table.tHead.rows; var c=r[row].cells; for (var i=0; i<c.length; i++) {
...
var c=table.config; var l=sortList.length; for (var i=0; i<l; i++) {
var s=sortList[i]; var o=c.headerList[s[0]]; o.count=s[1]; o.count++;
}
} function multisort(table, sortList, cache) {
if (table.config.debug) {
var sortTime=new Date();
} var dynamicExp='var sortWrapper = function(a,b) {'; var l=sortList.length; for (var i=0; i<l; i++) {
...
var a='a['+index+']'; var b='b['+index+']'; if (type=='text'&&direction=='asc') {
...
if (!this.tHead||!this.tBodies) return; var $this; var $document; var $headers; var cache; var config; var shiftDown=0; var sortOrder; this.config={}; config=$.extend(this.config, $.tablesorter.defaults, settings); $this=$(this); $.data(this, 'tablesorter', config); $headers=buildHeaders(this); this.config.parsers=buildParserCache(this, $headers); cache=buildCache(this); var sortCSS=[config.cssDesc, config.cssAsc]; fixColumnWidth(this); $headers.click(function(e) {
...
var s=config.sortList[j]; var o=config.headerList[s[0]]; if (s[0]==i) {
...
var l=parsers.length; var a=true; for (var i=0; i<l; i++) {
...
}}); $.fn.extend({tablesorter: $.tablesorter.construct}); var ts=$.tablesorter; ts.addParser({id: 'text', is: function(s) {
return true;
}, format: function(s) {
return $.trim(s.toLocaleLowerCase());
}, type: 'text'}); ts.addParser({id: 'digit', is: function(s, table) {
var c=table.config; return $.tablesorter.isDigit(s, c);
}, format: function(s) {
return $.tablesorter.formatFloat(s);
}, type: 'numeric'}); ts.addParser({id: 'currency', is: function(s) {
return /^[£$€?.]/.test(s);
}, format: function(s) {
return $.tablesorter.formatFloat(s.replace(new RegExp(/[£$€]/g), ''));
}, type: 'numeric'}); ts.addParser({id: 'ipAddress', is: function(s) {
return /^\d{2,3}[\.]\d{2,3}[\.]\d{2,3}[\.]\d{2,3}$/.test(s);
}, format: function(s) {
var a=s.split('.'); var r=''; var l=a.length; for (var i=0; i<l; i++) {
...
}, type: 'numeric'}); ts.addParser({id: 'url', is: function(s) {
return /^(https?|ftp|file):\/\/$/.test(s);
}, format: function(s) {
return jQuery.trim(s.replace(new RegExp(/(https?|ftp|file):\/\//), ''));
}, type: 'text'}); ts.addParser({id: 'isoDate', is: function(s) {
return /^\d{4}[\/-]\d{1,2}[\/-]\d{1,2}$/.test(s);
}, format: function(s) {
return $.tablesorter.formatFloat((s!='')?new Date(s.replace(new RegExp(/-/g), '/')).getTime():'0');
}, type: 'numeric'}); ts.addParser({id: 'percent', is: function(s) {
return /\%$/.test($.trim(s));
}, format: function(s) {
return $.tablesorter.formatFloat(s.replace(new RegExp(/%/g), ''));
}, type: 'numeric'}); ts.addParser({id: 'usLongDate', is: function(s) {
return s.match(new RegExp(/^[A-Za-z]{3,10}\.? [0-9]{1,2}, ([0-9]{4}|'?[0-9]{2}) (([0-2]?[0-9]:[0-5][0-9])|([0-1]?[0-9]:[0-5][0-9]\s(AM|PM)))$/));
}, format: function(s) {
return $.tablesorter.formatFloat(new Date(s).getTime());
}, type: 'numeric'}); ts.addParser({id: 'shortDate', is: function(s) {
...
}, type: 'numeric'}); ts.addParser({id: 'time', is: function(s) {
return /^(([0-2]?[0-9]:[0-5][0-9])|([0-1]?[0-9]:[0-5][0-9]\s(am|pm)))$/.test(s);
}, format: function(s) {
return $.tablesorter.formatFloat(new Date('2000/01/01 '+s).getTime());
}, type: 'numeric'}); ts.addParser({id: 'metadata', is: function(s) {
return false;
}, format: function(s, table, cell) {
var c=table.config; var p=(!c.parserMetadataName)?'sortValue':c.parserMetadataName; return $(cell).metadata()[p];
}, type: 'numeric'}); ts.addWidget({id: 'zebra', format: function(table) {
if (table.config.debug) {
var time=new Date();
} var $tr; var row=-1; var odd; $('tr:visible', table.tBodies[0]).each(function(i) {
