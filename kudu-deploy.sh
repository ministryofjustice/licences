...
installYarn () {
  # Already present?
  if which yarn.cmd 2>/dev/null >/dev/null; then
    echo "Yarn is installed globally"
  else
    # Present if we add the yarn installer's path
    export PATH="$HOME/.yarn/bin:$PATH"
    if which yarn 2>/dev/null >/dev/null; then
